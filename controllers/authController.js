...
export const register = catchAsyncErrors(async(req, res, next)=>{
    try{
       ...
       if(registrationAttemptsByUser.length >= 5){
        return next(new ErrorHandler("You have accessed the number of registration attempts. Please contact support", 400));
       }
...
export const forgotPassword = catchAsyncErrors(async(req, res, next)=>{
   ...
   try{
       await sendEmail({email: user.email, subject: "BoiGhor password recovery", message,})
       res.status(200).json({
         success: true,
